...
def init_params(d=100, d_in=2, d_out=2, d_incoming=15):
    ...
    W_hh = np.zeros((d, d))
    for i in range(d):
        W_hh[i, rng.choice(d, (d_incoming,), replace=False)] = 1 / np.sqrt(d_incoming) * rng.randn(d_incoming)

    u, _, v = np.linalg.svd(W_hh)
    W_hh = u.dot(v)
    W_hh = np.array(W_hh)
    W_hh = jnp.array(W_hh)
...
def predict(x, params, dt=1, tau=10, steps=450):
    ...
    def scan_f(h_lin, xs):
        h_lin = h_lin + dt/tau * (-h_lin + xs @ W_hx + jnp.tanh(h_lin) @ W_hh)
        h = jnp.tanh(h_lin)
        y_hat = h @ W_yh
...
def G(v, x, params, reg, loss='mse'):
    ...
    diag = jnp.concatenate((jnp.ones(2) / (2 * 450 * 500), 
...
def reduction_ratio(parameters, delta, x, y, grad, regularization):
    ...
    Gv = G(delta, x, parameters, regularization)
    loss_quadratic = 0.5 * delta.dot(Gv) + \
        delta.dot(grad)
